...
{
    public partial class Edit_ContractContact : System.Web.UI.Page
    {
        ...
        string result, Name;
        string Contractid;
        string trash, visible,  RenewContractExpired;
        ...
        public DataTable GetTaskbyModuleWise()
        {
            ...
            using (SqlConnection con = new SqlConnection(strconnect))
            {
                ...
                sqlCommand.Parameters.AddWithValue("@BelongTo", "Contract Partner");
        ...
        public void BindContracttype()//ddl
        {
            try
            {
                SqlConnection con = new SqlConnection(strconnect);
                SqlCommand cmd = new SqlCommand("SP_GetcontractTypeName", con);
                ...
                ddlContracttype.Items.Insert(0, new ListItem("Select Contractype", "0"));
            }
            catch (Exception ex)
            {
        ...
        public void BindPartnerName()//ddl
        {
            try
            {
                ...
                ddlpartnername.Items.Insert(0, new ListItem("Select Partner Name", "0"));
            }
            catch (Exception ex)
            {
                using (SqlConnection DeviceCon = new SqlConnection(strconnect))
        ...
        public void GetContractPartnerByID()
        {
            try
            {
                Contractid = HttpUtility.UrlDecode(Request.QueryString["ID"]);
                lblContractPartnerID.Text = Contractid;
                using (SqlConnection UserCon = new SqlConnection(strconnect))
                {
                    SqlCommand cmd = new SqlCommand("SP_GetContractPartnerByID", UserCon);
                    cmd.CommandType = System.Data.CommandType.StoredProcedure;
                    SqlDataAdapter sda = new SqlDataAdapter(cmd);

                    cmd.Parameters.AddWithValue("@id", lblContractPartnerID.Text);
                    DataTable dt = new DataTable();
                    sda.Fill(dt);
                    if (dt.Rows.Count > 0)
                    {
                        ddlpartnername.SelectedItem.Text = dt.Rows[0]["PartnerName"].ToString();
                        ...
                        lblpartnerFilename.Text = dt.Rows[0]["FileName"].ToString();//filepath add
                        if (trash == "True")
                        {
                        ...
                        if (visible == "True")
                        {
                            CheckHidePartner.Checked = true;
                        }
                        else
                        {
                            CheckHidePartner.Checked = false;
                        }
                    }
            ...
            try
            {
                if (Session["LoginType"] != null)
                {
                    ...
                    if (Session["LoginType"].ToString() == "Administrator")
                    {
                        ...
                        if (!IsPostBack)
                        {
                            //  txtStartDate.Text = Convert.ToString(DateTime.Now.Date);
                            BindPartnerName();
                            BindContracttype();
                            GetContractPartnerByID();
                            GetTaskbyModuleWise();
                        }
        ...
        protected void btnEditProject_Click1(object sender, EventArgs e)
        {
            try
            {
                ...
                if (CheckHidePartner.Checked == true)
                {
                    visible = "True";
                }
                else
                ...
                using (SqlConnection con = new SqlConnection(strconnect))
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_UpdateContractPartner", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Connection = con;
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@id", lblContractPartnerID.Text);
                    ...
                    cmd.Parameters.AddWithValue("@PartnerName", ddlpartnername.SelectedItem.Text);
                    cmd.Parameters.AddWithValue("@partnerID", ddlpartnername.SelectedValue);
                    ...
                    if (Result < 0)
                    {
                        ...
                        Response.Redirect("~/Contract_Contact.aspx?edit1=" + edit + "", false);
                    }
                    else
                    {
                        Toasteralert.Visible = false;
                        deleteToaster.Visible = true;
                        lblMesDelete.Text = "Partner Contract Details Not Edit Successfully";
                    }
                }
            }
            catch (Exception ex)
        ...
        //2
        protected void LinkButtoncontract_lastname_Click1(object sender, EventArgs e)
        {
        ...
        protected void Btn_Upload_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    string uploadDirectory = Server.MapPath("~/Contract_Partner_Details_Upload/");
                    ...
                    using (SqlConnection con = new SqlConnection(strconnect))
                    {
                        SqlCommand cmd = new SqlCommand("SP_UploadContractPartnerAttachmentFile", con);
                        cmd.Connection = con;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@id", lblContractPartnerID.Text);
                        ...
                        if (i < 0)
                        {
                            Toasteralert.Visible = false;
                            deleteToaster.Visible = true;
                            lblMesDelete.Text = "Partner Contract Details File Upload Successfully";
                            GetContractPartnerByID();
                        }
                        else
                        {
                ...
                try
                {
                    ...
                    if (NewEndDate.Date > NewStartDate.Date && RenewContractExpired == "true" || NewStartDate.Date == Today && RenewContractExpired == "true")
                    {
                        using (SqlConnection con = new SqlConnection(strconnect))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@contract_id", lblContractPartnerID.Text);
        ...
        protected void Linkbtnpdf_Click1(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (ddlpartnername.SelectedItem.Text == "Select Contractype")
                {
                    partner = "Contract Partner Details";                   
                }
                else
                {
                    partner = ddlpartnername.SelectedItem.Text;
                }
                //Set default file Name as current datetime
                Response.AddHeader("content-disposition", "attachment; filename=" + partner + ".pdf");
