...
{
    public partial class New_Office_Expenses : System.Web.UI.Page
    {
        ...
        public void Clear()
        {
            ...
            btnSaveOfficeExpense.Visible = true;
            bindItem();
            bindTax();
        }
        ...
        public void GetMaxID(string BillNo)
        {
            ...
            {
                SqlConnection conn = new SqlConnection(strconnect);
                SqlCommand cmd = new SqlCommand("SP_GeTMaxIDExpensesOffice", conn);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        protected void btnSaveOfficeExpense_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                try
                {
                    ...
                    SqlCommand UserCommand = new SqlCommand("SP_SaveOfficeExpenses", UserCon);
                    ...
                    if (Result > 0)
                    {
                        this.Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Office Expenses in Draft  Successfully!')", true);
                    }
                    else
                    {
                        this.Page.ClientScript.RegisterStartupScript(this.GetType(), "alert", "alert('Office Expenses Already Available!')", true);
                    }
                    ...
                    btnSaveOfficeExpense.Visible = false;
                }
                catch (Exception ex)
                {
                    using (SqlConnection DeviceCon = new SqlConnection(strconnect))
        ...
        protected void btnSaveAll_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Result < 0)
                {
                    ...
                    Response.Redirect("~/Office_Expences.aspx?svd1=" + save + "", false);
                }
                else
                {
                    Toasteralert.Visible = false;
                    deleteToaster.Visible = true;
                    lblMesDelete.Text = "Office Expenses Draft to Save Successfully";
                }
                GetMaxID(lblBillNoretrive.Text);
                Calculatefilldata();
                ViewFileExpensesDetails();
